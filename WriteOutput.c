...
void InitWriteOutput() { gettimeofday(&startTime, NULL); }
...
void WriteOutputf(FILE *f, int carID, char connector_type, int connectorID,
                  Action action) {
    unsigned long long time = GetTimestamp();
    pthread_mutex_lock(&mutexWrite);

    PrintThreadId(f);
    fprintf(f, "CarID: %d, Object: %c%d, time stamp: %llu, AID: %d ", carID,
            connector_type, connectorID, time, (int)action);
    switch (action) {
    case TRAVEL:
...
void WriteOutput(int carID, char connector_type, int connectorID,
                 Action action) {
